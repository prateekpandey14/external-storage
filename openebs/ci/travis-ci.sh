...
function dumpPoolLogs() {
  LC=$1
  POOLPOD=$(kubectl get pods -n openebs -o jsonpath='{.items[?(@.metadata.labels.openebs\.io/storage-pool-claim=="sparse-claim-auto")].metadata.name}')
  echo "------- cstor-pool logs -----"
	kubectl logs ${LC} $POOLPOD -n openebs -c cstor-pool
  printf "\n\n"
  echo "------- cstor-pool-mgmt logs -----"
  kubectl logs ${LC} $POOLPOD -n openebs -c cstor-pool-mgmt
...
echo "--------------------- openebs images ------------------------"
kubectl get pods -n openebs -o yaml

echo "-------- csp yaml --------"
kubectl get csp -o yaml
...
export POOLPOD=$(kubectl get pods -n openebs -o jsonpath='{.items[?(@.metadata.labels.openebs\.io/storage-pool-claim=="sparse-claim-auto")].metadata.name}')

"----------------------- cstor-pool-mgmt logs ----------------------------"
dumpPoolLogs -f
dumpPoolLogs -p
